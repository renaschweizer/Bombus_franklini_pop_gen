...
for (x in 1:nrow(samples)) {points(samples$longitude[x],samples$latitude[x],col="black",pch=21,cex=1)}
...
pdf(file="sequencing_metrics_illumina_element/bfranklini_raw_coverage_by_year.pdf")
...
print("Linear model of mean raw coverage vs. collection year")
summary(lm(meanCov.raw ~ CollectionYr,stats_meta))

# raw, cleaned reads by year

pdf(file="sequencing_metrics_illumina_element/bfranklini_mq20_reads_by_year.pdf")
...
print("Linear model of number of unique MQ20  paired-end reads vs. collection year")
...
We can also check the correlation between DP and specimen age, now that we've filtered the data. 

```{r Filtering exploration: specimen age vs depth of coverage after filtering, echo=FALSE}

# we will merge depth_4 wiith our meta information

depth_4_age <- merge(depth_4,stats_meta[,c(1,3)],by.x="INDV",by.y="SampleID" )

# plot the depth of coverage for filtered sites vs collection year 

t <- ggplot(depth_4_age, aes(x=CollectionYr, y=MEAN_DEPTH)) + 
  geom_point()+
  geom_smooth(method=lm) + labs(y = "Mean Depth of Coverage (Filtered, F4)", x="Collection Year")
t

pdf(file="sequencing_metrics_illumina_element/bfranklini_filt_F4_reads_by_year.pdf")
t
garbage <- dev.off()

# test whether there is a significant correlation 

summary(lm(MEAN_DEPTH ~ CollectionYr,depth_4_age))

# we will merge depth_5 wiith our meta information

depth_5_age <- merge(depth_5,stats_meta[,c(1,3)],by.x="INDV",by.y="SampleID" )

# plot the depth of coverage for filtered sites vs collection year 

u <- ggplot(depth_5_age, aes(x=CollectionYr, y=MEAN_DEPTH)) + 
  geom_point()+
  geom_smooth(method=lm) + labs(y = "Mean Depth of Coverage (Filtered, F5)", x="Collection Year")
u

pdf(file="sequencing_metrics_illumina_element/bfranklini_filt_F5_reads_by_year.pdf")
u
garbage <- dev.off()

# test whether there is a significant correlation 

summary(lm(MEAN_DEPTH ~ CollectionYr,depth_5_age))
...
# add in info from Lozier et al. 2023 for B. vancouverensis

new_row <- data.frame(Species = "Bombus_vancouverensis", Observed_π_species = 0.00303, Common_Name="Vancouver_Bumblebee",Kingdom="animal",Seq_Type="genome-wide", Source="Lozier_2023",Insect="yes")
johnson_het_add_means <- rbind(johnson_het_add_means, new_row)


# plot and group by Insect
i <- ggplot(johnson_het_add_means, aes(x=Observed_π_species,color=Insect,fill=Insect)) +
  geom_histogram(position="identity", alpha=0.75,bins=50) + geom_vline(xintercept=mean(genomehet_4$het),linetype="dashed", color = "black") + scale_x_continuous(trans = "log10",labels = function(x) format(x, scientific = FALSE)) + labs(x = "Genome-wide Heterozygosity",y="Frequency")+ geom_text(aes(x=mean(genomehet_4$het), label="B. franklini", y=5.3), colour="black", vjust = 1.2) + theme_bw() + scale_fill_manual(values = c("yes" = "#1f78b4", "no" = "#a6cee3")) + scale_color_manual(values = c("yes" = "#1f78b4", "no" = "#a6cee3"))
i
